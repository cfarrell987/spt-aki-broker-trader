...
async function main() 
{
    ...
    try 
    {
        ...
        // Purge "user\mods" folder from existing versions of the mod.
        const modEntries = await fs.promises.readdir(sptAkiModsLocation, { withFileTypes: true });
        for (const entry of modEntries)
        {
            if (entry.isDirectory())
            {
                if (entry.name.startsWith(getProjectNameWithoutVersion(packageJson)))
                {
                    const entryPath = path.join(sptAkiModsLocation, entry.name);
                    await fs.remove(entryPath);
                    logger.log("success", `Purged ${entryPath}`);
                }
            }
        }
        
        // Move server mod into SP Tarkov's "user/mods" directory.
        const modInstallDestination = path.join(sptAkiModsLocation, projectName);
        await fs.move(projectDir, modInstallDestination);
        logger.log("success", `Installed the server mod to ${modInstallDestination}`);

        // Purge "BepInEx\plugins" folder from existing client plugins of the mod.
        const pluginEntries = await fs.promises.readdir(sptAkiPluginsLocation, { withFileTypes: true });
        for (const entry of pluginEntries)
        {
            if (entry.isFile())
            {
                if (entry.name.startsWith(pluginBaseName) && entry.name.endsWith("dll"))
                {
                    const entryPath = path.join(sptAkiPluginsLocation, entry.name);
                    await fs.remove(entryPath);
                    logger.log("success", `Purged ${entryPath}`);
                }
            }
        ...
        const pluginInstallDestination = path.join(sptAkiPluginsLocation, pluginFullName);
        if (await fs.pathExists(pluginBuildLocation))
        {
            await fs.copy(pluginBuildLocation, pluginInstallDestination);
            logger.log("success", `Installed the client plugin to ${pluginInstallDestination}`);
        }
        else 
        {
